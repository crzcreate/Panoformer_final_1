...
class GetHf(nn.Module):
    def __init__(self, input_size, output_size):
        super(GetHf, self).__init__()  # 全连接层
        self.hf_max=4
    ...
    def forward(self, x):
        return (self.proj(x))*self.hf_max
...
class DepthOutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=1, kernel_size=3, stride=1, norm_layer=None, act_layer=None,
        ...
        self.proj = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channel),#这里最好不要relu,如果batchnorm的话，那就是相当于认为整个深度图都是一个分布的，这不合理，至少不是高斯分布
            nn.ReLU()
        )
...
class RateOutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=1, kernel_size=3, stride=1, norm_layer=None, act_layer=None,
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=1),
            nn.BatchNorm2d(out_channel),#这里也是一样，同一张图的rate的分布并不是同一个样子的
            nn.Sigmoid()
        )
        if act_layer is not None:
            self.proj.add_module(act_layer(inplace=True))
    ...
    def forward(self, x):
        ...
        mask = (x > self.threshold).float()# for 1024*512
        return mask
...
class Panoformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
        ...
        self.output_proj = DepthOutputProj(in_channel=2 * embed_dim, out_channel=1, kernel_size=3, stride=1,
                                      input_resolution=(img_size, img_size * 2))
        self.hf_proj=GetHf(input_size=4*embed_dim*img_size**2,output_size=1)
        # Encoder
    ...
    def forward(self, x):
        ...
        # Output Projection理论上这里的depth1应该小的地方是物体，所以应该乘以比较大的值，但是imrate是比较小的值，所以应该乘以（1-imrate）,第二种方案，imrate是0，1数值，会合理点，需要得到一个hf很重要,hf来个全连接吧
        item_rate = self.item_rate(deconv3)
        depth1 = self.output_proj(deconv3)
        #这里需要得到一个hf，这个很重要，而且是无监督状态的
        flattened=deconv3.view(deconv3.shape[0],-1)
        hf=self.hf_proj(flattened)
        layout_depth = inference(bon, cor, hf).cuda()
