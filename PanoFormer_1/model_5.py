...
class DepthOutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=1, kernel_size=3, stride=1, norm_layer=None, act_layer=None,
        ...
        self.proj = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channel),
            nn.GELU()#这里其实如果不加relu的话，就可以学到比较小的值了,或者尝试gelu函数，能保证学到比较小的深度，应该是这样
        )
...
class CustomSigmoid(nn.Module):
    ...
    def forward(self, x):
        return 1 / (1 + torch.exp(-3*x))#先设置为5运行几个epoch看看效果，如果没有norm，那么应该把这个调小一点，应该会好收敛一点？
class RateOutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=1, kernel_size=3, stride=1, norm_layer=None, act_layer=None,
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=1),
            # nn.BatchNorm2d(out_channel),#这个norm要不要还不清楚,鉴定完毕，这个relu与否不影响，可能是这个sigmoid，可以看看itemrate的含金量
            CustomSigmoid(),
...
class Panoformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
        ...
        #下面是itemrate的实现，类似depth的估计，只是最后加上sigmoid函数，或者类sigmoid函数
        self.conv_rate = BasicPanoformerLayer(dim=embed_dim * 16,
                                         output_dim=embed_dim * 16,
                                         input_resolution=(img_size // (2 ** 4), img_size * 2 // (2 ** 4)),
                                         depth=depths[4],
                                         num_heads=num_heads[4],
        ...
        # Decoder
        self.upsample_0_rate = upsample(embed_dim * 16, embed_dim * 8,
                                   input_resolution=(img_size // (2 ** 4), img_size * 2 // (2 ** 4)))
        self.decoderlayer_0_rate = BasicPanoformerLayer(dim=embed_dim * 16,
                                                   output_dim=embed_dim * 16,
                                                   input_resolution=(img_size // (2 ** 3), img_size * 2 // (2 ** 3)),
                                                   depth=depths[5],
                                                   num_heads=num_heads[5],
                                                   ...
                                                   token_projection=token_projection, token_mlp=token_mlp,
                                                   se_layer=se_layer, ref_point=self.ref_point32x64, flag=1)
        self.upsample_1_rate = upsample(embed_dim * 16, embed_dim * 4,
                                   input_resolution=(img_size // (2 ** 3), img_size * 2 // (2 ** 3)))
        self.decoderlayer_1_rate = BasicPanoformerLayer(dim=embed_dim * 8,
                                                   output_dim=embed_dim * 8,
                                                   input_resolution=(img_size // (2 ** 2), img_size * 2 // (2 ** 2)),
                                                   depth=depths[6],
                                                   num_heads=num_heads[6],
                                                   ...
                                                   token_projection=token_projection, token_mlp=token_mlp,
                                                   se_layer=se_layer, ref_point=self.ref_point64x128, flag=1)
        self.upsample_2_rate = upsample(embed_dim * 8, embed_dim * 2,
                                   input_resolution=(img_size // (2 ** 2), img_size * 2 // (2 ** 2)))
        self.decoderlayer_2_rate = BasicPanoformerLayer(dim=embed_dim * 4,
                                                   output_dim=embed_dim * 4,
                                                   input_resolution=(img_size // 2, img_size * 2 // 2),
                                                   depth=depths[7],
                                                   num_heads=num_heads[7],
                                                   ...
                                                   token_projection=token_projection, token_mlp=token_mlp,
                                                   se_layer=se_layer, ref_point=self.ref_point128x256, flag=1)
        self.upsample_3_rate = upsample(embed_dim * 4, embed_dim, input_resolution=(img_size // 2, img_size * 2 // 2))
        self.decoderlayer_3_rate = BasicPanoformerLayer(dim=embed_dim * 2,
                                                   output_dim=embed_dim * 2,
                                                   input_resolution=(img_size, img_size * 2),
                                                   depth=depths[8],
                                                   num_heads=num_heads[8],
    ...
    def forward(self, x):
        ...
        #decoder这里实现一个概率的预测，用来分配具体的置信度
        # Decoder的item_rate部分
        conv4_rate = self.conv(pool3)
        up0_rate = self.upsample_0(conv4_rate)
        deconv0_rate = torch.cat([up0_rate, conv3], -1)
        deconv0_rate = self.decoderlayer_0(deconv0_rate)

        up1_rate = self.upsample_1(deconv0_rate)
        deconv1_rate = torch.cat([up1_rate, conv2], -1)
        deconv1_rate = self.decoderlayer_1(deconv1_rate)

        up2_rate = self.upsample_2(deconv1_rate)
        deconv2_rate = torch.cat([up2_rate, conv1], -1)
        deconv2_rate = self.decoderlayer_2(deconv2_rate)

        up3_rate = self.upsample_3(deconv2_rate)
        deconv3_rate = torch.cat([up3_rate, conv0], -1)
        deconv3_rate = self.decoderlayer_3(deconv3_rate)

        # Output Projection
        item_rate = self.item_rate(deconv3_rate)#这里可以考虑不用deconv3，用一个其他的模块，或者简单一点的模块得到itemrate,用deconv3_rate来独立计算
        #判断itemrate的方差，理论上本质应该是这里的分布应该有一个断崖形式的
        # item_var=torch.var(item_rate)
        depth1 = self.output_proj(deconv3)
        layout_depth = inference(bon, cor, depth1).cuda()
        depth2 = layout_depth.unsqueeze(1)
        y=depth1*item_rate+depth2*(1-item_rate)#这里应该加个这个
        outputs = {}
        outputs["pred_depth"] = y
        #用来看看depth1还有itemrate以及y的问题
        # item_rate_ = np.array(item_rate.detach().cpu())#这里可以考虑给itemrate可视化一下，看看学的效果如何
        # outputs["item_rate"]=item_rate
        # max_rate=np.sum(item_rate_>0.7)/(512*1024)
        # min_rate=np.sum(item_rate_<0.3)/(512*1024)
        # depth1_ = np.array(depth1.detach().cpu())
        # mean_depth1=depth1_.mean()#这里其实发现大致depth1只会有0.5左右
