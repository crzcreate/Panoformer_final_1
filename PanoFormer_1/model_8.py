...
class DepthOutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=1, kernel_size=3, stride=1, norm_layer=None, act_layer=None,
        ...
        self.proj = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channel),
            nn.ReLU()#这里其实如果不加relu的话，就可以学到比较小的值了,或者尝试gelu函数，能保证学到比较小的深度，应该是这样
        )
...
class CustomSigmoid(nn.Module):
    ...
    def forward(self, x):
        return 1 / (1 + torch.exp(-5*x))#先设置为5运行几个epoch看看效果，如果没有norm，那么应该把这个调小一点，应该会好收敛一点？
class RateOutputProj(nn.Module):
    ...
    def forward(self, x):
        ...
        H, W = self.input_resolution
        x = x.transpose(1, 2).view(B, C, H, W)
        x = F.interpolate(x, scale_factor=2,mode='nearest')  # for 1024*512#这里是不能用最近邻插值的要用linear,实在不行可以卷积一下，如果只是线性插值是不是可能不太行,方法1反卷积，方法2线性插值，最近邻插值
        # x = F.pad(x, (3 // 2, 3 // 2, 0, 0), mode='circular')  # width
        # x = F.pad(x, (0, 0, 3 // 2, 3 // 2))
        x = self.proj(x)
        if self.norm is not None:
...
class Panoformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
                 embed_dim=32, depths=[2, 2, 2, 2, 2, 4, 4, 4, 4], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],#这里考虑给这个selfattention多搞几层，因为本身encoder部分相当于不见了，想要更多的数据效果，那只能多来几个decoder了
        ...
        dec_dpr = [x.item() for x in torch.linspace(drop_path_rate, 0, sum(depths[self.num_enc_layers+1:]))]
    ...
    def forward(self, x):
        ...
        # # max_rate=np.sum(item_rate_>0.7)/(2*512*1024)
        # # min_rate=np.sum(item_rate_<0.3)/(2*512*1024)
