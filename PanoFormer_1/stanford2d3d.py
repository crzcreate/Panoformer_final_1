...
class Stanford2D3D(data.Dataset):
    ...
    def __init__(self, root_dir, list_file, height=512, width=1024, disable_color_augmentation=True,
                 disable_LR_filp_augmentation=True, disable_yaw_rotation_augmentation=True, is_training=False):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        # i_img_mask = torch.tensor(np.logical_and(rgb[..., 0] == 0, rgb[..., 1] == 0, rgb[..., 2] == 0) * 1)#这里的mask说不定有奇效
        ...
        mask[0:int(512*0.25), :] = 0
        mask[512-int(512*0.25):512, :] = 0#这个mask是不是还能再改改，按照fredsnet的走
        #第二种mask方法
        # mask = torch.ones([512, 1024])-i_img_mask
        #第三种最变态的方法
        # mask = torch.ones([512, 1024])
        # mask[0:int(512 * 0.40), :] = 0
        # mask[512 - int(512 * 0.40):512, :] = 0


        inputs["gt_depth"] = torch.from_numpy(np.expand_dims(gt_depth, axis=0)).type(torch.float64)
